...
notation IMP_VariableI ("_\<^bold>[_\<^bold>]" [100,0] 100)
...
definition "var x \<equiv> (\<lambda>_. 0)(0:=x)"

lemma var_idx[simp]: "var x 0 = x" by (simp add: var_def)
lemma var_subst[simp]: "(var x)(0:=y) = var y" by (simp add: var_def)
lemma var_inj[simp]: "var x = var y \<longleftrightarrow> x=y" 
  unfolding var_def
  apply (auto)
  by (meson fun_upd_eqD)
lemma var_fold[simp]: "(\<lambda>_. 0)(0:=x) = var x"
  by (simp add: var_def)
...
method Bs_simp = simp add: var_def del: var_fold
...
lemma thin_eqon: "(s::state) = t on X \<Longrightarrow> P \<Longrightarrow> P" .
