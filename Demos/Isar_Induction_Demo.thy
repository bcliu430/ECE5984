...
text \<open> Induction with long-goal format \<close>

lemma split_list: 
  assumes "x \<in> set xs"  
  shows "\<exists>ys zs. xs = ys @ x # zs"
  using assms -- \<open>Give assumptions to induction method. Required if they contain induction/arbitrary variables! \<close>
proof (induction xs)
...
next
  ...
  thm assms -- \<open>Also available, but makes no sense here, as it refers to different variables!\<close>
...
lemma "x \<in> set xs \<Longrightarrow> \<exists>ys zs. xs = ys @ x # zs"
proof (induction xs)
  case Nil thus ?case by simp
next
  case (Cons a xs)
...
next
  ...
  -- \<open>Note, name evSS for the intro-rule is hidden by case-name! Use ev.evSS or ev.intros(1) instead. \<close>
  thm ev.evSS 
  thm ev.intros
