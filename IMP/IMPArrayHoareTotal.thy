...
begin
  ...
  lemma if_rule:  
    ...
    shows "\<Turnstile> {\<lambda>s. COND (bval b s) (P1 s) (P2 s)} IF b THEN c\<^sub>1 ELSE c\<^sub>2 {Q}"
...
definition
hoaret_valid :: "assn \<Rightarrow> com \<Rightarrow> (assn) \<Rightarrow> bool" ("\<Turnstile>\<^sub>t {(1_)}/ (_)/ {(1_)}" 50) where
"\<Turnstile>\<^sub>t {P}c{Q} = (\<forall>s. P s \<longrightarrow> (\<exists>t. (c,s) \<Rightarrow> t \<and> Q t))"

context
  notes [simp] = hoaret_valid_def
begin
  lemma hoaret_validI: 
    assumes "\<And>s. P s \<Longrightarrow> \<exists>t. (c,s) \<Rightarrow> t \<and> Q t"
  ...
  lemma assign_trule: "\<Turnstile>\<^sub>t {\<lambda>s. P (s( x := (s x)(aval i s := aval a s) )) } x[i]::=a {P}"
    by (auto)
    
  lemma assign_trule_workaround: (* Workaround to strange unification problem *)
    assumes "\<And>s. P (s( x := (s x)(aval i s := aval a s))) = P' s"
    shows "\<Turnstile>\<^sub>t {P'} Assign x i a {P}"
    using assms
    by (auto)
    

  lemma clear_trule: "\<Turnstile>\<^sub>t {\<lambda>s. P (s( x := (\<lambda>_. 0) )) } CLR x {P}"
    by (auto)
  ...
  lemma if_trule:  
    ...
    shows "\<Turnstile>\<^sub>t {\<lambda>s. COND (bval b s) (P1 s) (P2 s)} IF b THEN c\<^sub>1 ELSE c\<^sub>2 {Q}"
    using assms unfolding COND_def by force
  
  lemma basic_while_trule:
    assumes WF: "wf R"
    assumes S: "\<And>s\<^sub>0. \<Turnstile>\<^sub>t {\<lambda>s. P s \<and> bval b s \<and> s=s\<^sub>0} c {\<lambda>s. P s \<and> (s,s\<^sub>0)\<in>R}"
    shows "\<Turnstile>\<^sub>t {P} WHILE b DO c {\<lambda>s. P s \<and> \<not> bval b s}"
  proof (rule hoaret_validI)  
    fix s
    assume I0: "P s"
    with WF show "\<exists>t. (WHILE b DO c, s) \<Rightarrow> t \<and> P t \<and> \<not> bval b t"
    proof (induction s rule: wf_induct_rule)
      case (less s)
      show ?case proof (cases "bval b s")
        case True
        with S less.prems obtain s' where "(c,s) \<Rightarrow> s'" "P s'" "(s',s)\<in>R"
          by auto
        with less.IH obtain t where "(WHILE b DO c, s') \<Rightarrow> t" "P t" "\<not> bval b t"
          by blast
  ...
  text \<open>We combine the while-rule with a consequence rule, to
    make it more usable in verification condition generation.
    
    Moreover, we write \<open>Ph s s\<close> as \<open>id (Ph s) s\<close>, to persuade 
    Isabelle's unification algorithm not to generate flex-flex pairs,
    which will make it explode later. 
    The initial goal simplification of the VCG will remove the \<open>id\<close>, once
    \<open>Ph\<close> has been instantiated.
    \<close>
  
  (* Explicit backwards derivation *)
  lemma while_trule:
    fixes m :: "state \<Rightarrow> nat"
    assumes "wf R"
    assumes "\<And>s\<^sub>0. \<Turnstile>\<^sub>t {Ph s\<^sub>0} c {\<lambda>s. P s \<and> (s, s\<^sub>0) \<in> R}"
    assumes "\<And>s. \<lbrakk>P s; bval b s\<rbrakk> \<Longrightarrow> id (Ph s) s"
    assumes "\<And>s. \<lbrakk>P s; \<not>bval b s\<rbrakk> \<Longrightarrow> Q s"
    shows "\<Turnstile>\<^sub>t {P} WHILE b DO c {Q}"
    apply (rule conseq_post_trule[where Q="\<lambda>s. P s \<and> \<not>bval b s"])
    ...
    apply (rule assms)
    apply clarify
    apply (rule assms[simplified]; simp)
    done
...
subsection \<open>Variable Binding\<close>
  definition imap :: "vname \<Rightarrow> ('a \<Rightarrow> int \<Rightarrow> val) \<Rightarrow> 'a \<Rightarrow> state \<Rightarrow> bool" where "imap x ty v s \<equiv> s x = ty v"

  definition VAR_BIND :: "vname \<Rightarrow> _ \<Rightarrow> ('a \<Rightarrow> assn) \<Rightarrow> assn" where
    "VAR_BIND x ty Q s \<equiv> \<exists>v. imap x ty v s \<and> Q v s"
  ...
  lemma imapD[vcg_vars_mksimps]: "imap x ty a s \<Longrightarrow> s x = ty a" by (auto simp: imap_def)
  
  lemma imap_subst[simp]: 
    "x\<noteq>y \<Longrightarrow> imap x ty v (s(y:=b)) \<longleftrightarrow> imap x ty v s"
    "imap x ty v (s(x:=b)) \<longleftrightarrow> b = ty v"
    unfolding imap_def by auto

  lemma VAR_BIND_elim[elim!]:
    assumes "VAR_BIND x ty (\<lambda>v. Q v) s"
    obtains v where "imap x ty v s" "Q v s"
    using assms unfolding VAR_BIND_def by auto

  lemma VAR_BIND_FINAL_elim[elim!]: 
  ...
  method vcg_resolve_bindings = 
    vcg_try_resolve_bindings; rule VAR_BIND_FINALI
...
term "vars a (b : var) c (x as xx) (''xx.d'' as xxx) (''__y'' as yy : id) in P a b c"
...
definition AWHILET :: "(state\<times>state) set \<Rightarrow> assn \<Rightarrow> bexp \<Rightarrow> com \<Rightarrow> com" ("(VAR _ INVAR _ WHILE _/ DO _)"  [0, 0, 61] 61)
...
lemma awhile_trule:
  ...
  assumes "\<And>s. \<lbrakk>P s; bval b s\<rbrakk> \<Longrightarrow> id (Ph s) s"
  assumes "\<And>s. \<lbrakk>P s; \<not>bval b s\<rbrakk> \<Longrightarrow> Q s"
  shows "\<Turnstile>\<^sub>t {P} VAR R INVAR P WHILE b DO c {Q}"
  unfolding AWHILET_def
  by (rule while_trule[OF assms])
  
  
subsection \<open>Basic VCG\<close>  

lemmas vcg_rules = skip_rule assign_rule clear_rule seq_rule if_rule awhile_rule
lemmas vcg_trules = skip_trule assign_trule clear_trule seq_trule if_trule awhile_trule conjI
...
method rule_wrapper methods m = m; (vcg_is_no_hoare_triple, rule VC_D)?

method vcg_rule (* Apply Hoare-rule and tag side conditions *)
  = rule_wrapper\<open>rule vcg_rules vcg_trules (*| (rule assign_trule_workaround, (simp;fail))*)\<close>
...
method vcg_prepare_vc -- \<open>Applied to prepare verification condition. May solve it.\<close>
  = rule VC_I; (vcg_resolve_bindings; vcg_remove_binding_assms | vcg_try_resolve_bindings)
  
method vcg_step_solver methods solver 
  = vcg_prepare_goal; ( vcg_rule | vcg_prepare_vc;solver )
method vcg_default_solver = clarsimp?; (safe?;clarsimp?;fail)?

method vcg_step = vcg_step_solver\<open>vcg_default_solver\<close>
method vcg_ctd = (vcg_step)+

method vcg_init = rule_wrapper\<open>rule conseq_pre_rule conseq_pre_trule\<close>
